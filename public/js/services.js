 ...
 * ============================================================================
 * Module:      services.js
 * Description: Systemd service manager, unit state controls (start/stop/restart/reload), and live journalctl modal.
 *
 * @author      Najmul Islam
    ...
    async loadServices() {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.updateCounters();
                if (this.tableBody) {
                    const isAuthMismatch = data.token_mismatch || (sId !== 'local-master' && (errMsg.includes('Unauthorized') || res.status === 401));
                    if (isAuthMismatch) {
                        this.tableBody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 2.5rem 1.5rem;">
                            <div style="max-width: 620px; margin: 0 auto; background: var(--bg-card); border: 1px solid rgba(234,179,8,0.5); border-radius: 8px; padding: 1.5rem; text-align: left;">
                                ...
                                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem;">
                                    ...
                                    <button class="btn btn-sm btn-secondary" onclick="window.systemdMgr && window.systemdMgr.loadServices()">🔄 Retry Connection</button>
                                </div>
                            </div>
                        </td></tr>`;
                    } else {
